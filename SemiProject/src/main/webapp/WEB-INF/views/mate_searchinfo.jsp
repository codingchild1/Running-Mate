   		...
   		/*  $('.search').click(function(){
			...
			console.log($('#input').val())
			  $.ajax({
	        		type:"post",
	        		async:false,
	        		url:"http://localhost:8090/searchinfo",
	        		dataType:"json",
	        		...
	        		success: function(data, textStatus){
	        			console.log(data);
	        			var accEle = "";
	        			for(let groupandmate of data){
	        				 accEle += '<span>'+groupandmate.title
	        				accEle += '<input>'+groupandmate.title
	        				accEle += '<input>'+groupandmate.id
	        				accEle += '<input>'+groupandmate.likeno
	        				accEle += '<input>'+groupandmate.no
	        				accEle += '<button>'+groupandmate.type
	        			}
	        			$('.register').detach();
	        			$('.container1').append(accEle);
	        			alert("성공.");
	        		},
	        		error:function(data, textStatus){
	        			alert("실패");
